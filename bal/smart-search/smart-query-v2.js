...
const SmartQueryV2 = (function(){
  ...
  function answerQuestionImpl(questionId){
    ...
    if(q._categoryId === "per_student"){
      ...
      switch(q.id){
        ...
        case "rank_movement_student":
          ...
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "growth_rate_student":
          if(a.growthRate==null) return { ok:false, text: q.unavailableMessage || "Needs at least 2 tests recorded to compute a growth rate." };
          vars.growthRate = a.growthRate;
          vars.growthNote = a.growthRate>=15 ? "That's a strong upward trend." :
                             a.growthRate<=-15 ? "That's a concerning drop worth a check-in." :
                             "Fairly steady, no big swing either way.";
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "grade_student":
          vars.grade = a.grade; vars.overallAvg = a.overallAvg;
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "overall_avg_student":
          vars.overallAvg = a.overallAvg; vars.grade = a.grade;
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "attendance_student":
          vars.totalAbsent = a.totalAbsent||0;
          vars.attendanceNote = !a.totalAbsent ? "Perfect attendance so far." :
                                 a.totalAbsent>=5 ? "That's a notable number of absences — worth a check-in." :
                                 "Nothing alarming yet.";
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "best_worst_test_student":
          if(!a.bestTest && !a.worstTest) return { ok:false, text: q.unavailableMessage || "Needs at least 2 tests recorded to compare." };
          vars.bestTestName = a.bestTest ? a.bestTest.name : "—"; vars.bestTestPct = a.bestTest ? a.bestTest.pct : "—";
          vars.worstTestName = a.worstTest ? a.worstTest.name : "—"; vars.worstTestPct = a.worstTest ? a.worstTest.pct : "—";
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        // These four surface the SAME pre-generated text already shown on
        // the Individual-mode "Recommendations"/"Progress Report" tabs
        // (compute-stats.js sets these on st.analysis via the generators
        // in render-i18n.js) — zero new computation, just exposing existing
        // text through Smart Search too.
        case "home_plan_student":
          if(!a.homePlan) return { ok:false, text: q.unavailableMessage || "No home plan generated yet." };
          vars.homePlan = a.homePlan;
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "school_plan_student":
          if(!a.schoolPlan) return { ok:false, text: q.unavailableMessage || "No school-side plan generated for this student." };
          vars.schoolPlan = a.schoolPlan;
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "parent_message_student":
          if(!a.parentMessage) return { ok:false, text: q.unavailableMessage || "No parent message generated yet." };
          vars.parentMessage = a.parentMessage;
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
        case "trend_facts_student":
          if(!a.trendFacts) return { ok:false, text: q.unavailableMessage || "Not enough test history yet." };
          vars.trendFacts = a.trendFacts;
          return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
      }
    }

    // Class-wide flag-list questions ("Who Needs Help" / "Top Performers"
    // buckets) — filters the same st.flags[] array + type sets those
    // buckets already use (BUCKET_HELP_FLAG_TYPES/BUCKET_TOP_FLAG_TYPES in
    // ui/common/render-buckets.js), duplicated here as a small local
    // constant rather than importing UI code into the BAL layer.
    if(q.id === "who_needs_help" || q.id === "who_is_excelling"){
      const HELP_TYPES = ["at-risk","first-below-pass","declining","sharp-drop","absent","volatile","burnout","data-gap","plateau","peer-outlier-low"];
      const TOP_TYPES = ["improving","resilient","peer-outlier-high"];
      const types = q.id === "who_needs_help" ? HELP_TYPES : TOP_TYPES;
      const students = (window.APP && APP.students) || [];
      const matches = students.filter(st => (st.flags||[]).some(f => types.indexOf(f.type)!==-1));
      if(!matches.length) return { ok:false, text: q.emptyMessage || "No students currently match." };
      vars.count = matches.length;
      vars.list = matches.slice(0,8).map(st => {
        const reasons = (st.flags||[]).filter(f=>types.indexOf(f.type)!==-1).map(f=>f.label);
        return `${st.name} (${reasons.join(", ")})`;
      }).join("; ");
      return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
    }
    ...
    if(q.id === "subject_breakdown_all" && Array.isArray(value)){
      if(!value.length) return { ok:false, text: q.emptyMessage || "No subject data available." };
      const sorted = [...value].sort((a,b)=>b.avgClass-a.avgClass);
      vars.breakdownList = sorted.map(s=>`${s.subject}: ${s.avgClass}%`).join(", ");
      return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
    }
    ...
    if(q.computeKey === "students[].analysis.growthRate" && Array.isArray(value)){
      if(!value.length) return { ok:false, text: q.emptyMessage || "Growth rate needs at least 2 tests recorded." };
      const sorted = [...value].sort((a,b)=>b.value-a.value);
      vars.leader = sorted[0].student.name; vars.leaderRate = sorted[0].value;
      vars.topList = sorted.slice(0,5).map(r=>`${r.student.name} (${r.value>=0?"+":""}${r.value}%)`).join(", ");
      return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
    }

    if(q.computeKey === "students[].analysis.overallAvg" && Array.isArray(value)){
      if(!value.length) return { ok:false, text: q.emptyMessage || "No score data available yet." };
      const sorted = [...value].sort((a,b)=>b.value-a.value);
      const topScore = sorted[0].value;
      vars.topperName = sorted.filter(r=>r.value===topScore).map(r=>r.student.name).join(", ");
      vars.topperScore = topScore;
      return { ok:true, text: fillTemplate(q.answerTemplate, vars) };
    }
  ...
  // Every student whose name shares at least one token (exact or small-typo
  // fuzzy match) with the query, ranked by how many of their own name
  // tokens matched (first+last both matching beats a shared surname alone
  // — see the matchCount/matchedLen tie-break below) and how much of their
  // name matched in total. Returns ALL candidates, not just the top one —
  // callers decide what "ambiguous" means for their situation (see
  // resolveStudentByName() and match() below, which use this differently).
  function findStudentCandidates(queryTokens){
    if(!window.APP || !APP.students || !APP.students.length) return [];
    const out = [];
    APP.students.forEach(function(s){
      if(!s || !s.name) return;
      let matchCount = 0, matchedLen = 0;
      tokenize(s.name).forEach(function(nt){
        // Exact match first; a small typo in a name token (fuzzyIncludes,
        // same tolerance already used for question-vocabulary matching
        // below — distance-1 under 7 chars, distance-2 at 7+, never
        // attempted under 5 chars) falls back to a fuzzy check so "Preya"
        // still resolves to "Priya". This does NOT fix every typo (a
        // 2-letter transposition like "Hedge" for "Hegde" is 2 edits on a
        // 5-char word, over that word length's distance-1 threshold — a
        // deliberately conservative line so short names don't fuzzy-match
        // an unrelated real word), but it does catch the common
        // single-letter-off case the exact-only version caught nothing of.
        if(nt.length >= 3 && (queryTokens.indexOf(nt) !== -1 || fuzzyIncludes(nt, queryTokens))){
          matchCount++; matchedLen += nt.length;
        }
      });
      if(matchCount > 0) out.push({ student: s, matchCount: matchCount, matchedLen: matchedLen });
    });
    out.sort(function(a,b){
      return b.matchCount - a.matchCount || b.matchedLen - a.matchedLen;
    });
    return out;
  }

  // Single best guess — kept for anything that only ever wants one answer
  // and is fine guessing on a genuine tie (nothing currently calls this;
  // match() below uses findStudentCandidates() directly instead, precisely
  // BECAUSE it needs to detect and ask about ties rather than guess).
  function resolveStudentByName(queryTokens){
    const candidates = findStudentCandidates(queryTokens);
    return candidates.length ? candidates[0].student : null;
  }
  ...
  // "How many students named X" / "count of X" — a genuinely different
  // question shape from everything else this engine answers: it's a live
  // roster search (every student whose name contains the searched
  // fragment), not a lookup against the question bank, and the honest
  // answer to a count question is "here's everyone that matched", not a
  // single best guess. Checked ahead of everything else in match() so it
  // short-circuits before the name-resolution/scoring pipeline below ever
  // has a chance to (wrongly) collapse a "how many" question down to one
  // student.
  const COUNT_STOPWORDS = new Set(["how","many","count","number","of","are","is","there","student","students","name","named","in","this","class","the","a","an"]);
  function detectNameCountQuery(qTokens){
    const hasCountIntent = (qTokens.indexOf("how")!==-1 && qTokens.indexOf("many")!==-1) || qTokens.indexOf("count")!==-1 || qTokens.indexOf("number")!==-1;
    if(!hasCountIntent) return null;
    if(!window.APP || !APP.students || !APP.students.length) return null;
    const candidateTokens = qTokens.filter(t => t.length>=3 && !COUNT_STOPWORDS.has(t));
    for(let i=0;i<candidateTokens.length;i++){
      const t = candidateTokens[i];
      const matches = APP.students.filter(function(s){
        if(!s || !s.name) return false;
        return tokenize(s.name).some(function(nt){ return nt===t || fuzzyIncludes(nt,[t]); });
      });
      if(matches.length) return { token: t, matches: matches };
    }
    return null;
  }

  // "class-" / "student-" prefix — explicit scoping. Typing e.g.
  // "student- is Priya improving" or "class- weakest subject" skips the
  // ambiguous parts of matching entirely instead of just biasing the
  // score: "class-" skips the per-student name scan altogether
  // (findStudentCandidates() below is an O(students × query tokens) fuzzy
  // scan — real savings on a large roster), and "student-" narrows scoring
  // to the per_student question set only, so a class-wide keyword can
  // never accidentally outrank the per-student answer that was actually
  // asked for. Purely a fast-path/precision aid on top of the existing
  // free-text matching — omitting the prefix still works exactly as
  // before.
  const PREFIX_RE = /^\s*(class|student)\s*-\s*/i;
  function stripScopePrefix(text){
    const m = PREFIX_RE.exec(text||"");
    return m ? { scope: m[1].toLowerCase(), rest: text.slice(m[0].length) } : { scope: null, rest: text };
  }

  function match(queryText, limit){
    limit = limit || 5;
    if(!_bank) return { ok:false, results:[], deflected:false, text:srT("smart_question_bank_not_loaded") };
    const { scope: explicitScope, rest: effectiveQueryText } = stripScopePrefix(queryText);
    const qTokens = tokenize(effectiveQueryText);
    if(!qTokens.length) return { ok:false, results:[], deflected:false, text:"" };

    const countQuery = explicitScope ? null : detectNameCountQuery(qTokens);
    if(countQuery){
      const names = countQuery.matches.map(function(s){ return s.name; }).join(", ");
      const text = countQuery.matches.length===1
        ? srT("smart_name_count_one",{token:countQuery.token,names:names})
        : srT("smart_name_count_many",{n:countQuery.matches.length,token:countQuery.token,names:names});
      return { ok:true, results:[], deflected:false, text: text };
    }

    let namedStudent = null, candidates;
    if(explicitScope === "class"){
      _lastMatchedStudent = null;
      candidates = flatQuestions().filter(q => q._categoryId==="class_management" && evalRequires(q._requires) && questionAiFeatureOk(q) && (!q._scope.length || q._scope.indexOf(currentMode())!==-1));
    } else {
      // FIX (report: chat only ever surfaces class-wide questions, never
      ...
      const namedCandidates = (explicitScope==="student" || currentMode()==="institution") ? findStudentCandidates(qTokens) : [];
      // Ambiguous on purpose, not a guess: TWO+ students can genuinely tie
      // (report: two students both surnamed "Hegde", both matching only on
      // that one shared token) — picking whichever happened to be scanned
      // first silently answers about the wrong child. Report asked
      // explicitly for "no assumption, ask me instead" — so a genuine tie
      // for the top spot stops here and asks which student was meant,
      // rather than falling through to the scoring pipeline below at all.
      const topTier = namedCandidates.length && namedCandidates.filter(function(c){
        return c.matchCount===namedCandidates[0].matchCount && c.matchedLen===namedCandidates[0].matchedLen;
      });
      if(topTier && topTier.length>1){
        _lastMatchedStudent = null;
        return {
          ok:true,
          results:[],
          deflected:false,
          ambiguousStudents: topTier.map(function(c){ return { id:c.student.id, name:c.student.name }; }),
          text: srT("smart_ambiguous_student",{names:topTier.map(function(c){ return c.student.name; }).join(", ")})
        };
      }
      namedStudent = namedCandidates.length ? namedCandidates[0].student : null;
      _lastMatchedStudent = namedStudent;
      if(explicitScope === "student"){
    ...
    if(!results.length){
      // A named student's question didn't match any specific per_student
      // template closely enough to score (e.g. just "Pooja Hegde" alone,
      // or "tell me about Priya") — give a real answer about THAT student
      // right away instead of a wall of suggestion chips that don't even
      // mention them (report: typing a student's name produced generic,
      // unrelated-looking class questions with no way to just get info on
      // the person typed). Composed from the same real, already-computed
      // per_student answers those specific canned questions give (rank
      // gap, trend, weakest subject) — no fabricated text, just answering
      // three of the model's own questions on the student's behalf and
      // joining the results. _lastMatchedStudent is already set above, so
      // answerQuestionImpl's per_student branch resolves the right person.
      if(namedStudent){
        const overviewParts = ["rank_gap","trend","weakest_subject_student"]
          .map(function(id){ return answerQuestionImpl(id); })
          .filter(function(r){ return r.ok; })
          .map(function(r){ return r.text; });
        if(overviewParts.length){
          return { ok:true, results:[], deflected:false, text: overviewParts.join(" ") };
        }
      }
      ...
      if((namedStudent || explicitScope || !isOutOfDomain(qTokens)) && candidates.length){
        return {
          ok:true,
  ...
  // Re-runs an ambiguous query once the person has picked which student
  // they meant (see match()'s ambiguousStudents branch above) — same
  // scoring pipeline as match(), just with student resolution skipped
  // entirely in favor of the given id, so a name that's ALSO ambiguous on
  // its own ("Hegde" matches two students) can't re-trigger the same
  // ambiguity check after the person already answered it once.
  function matchForStudent(queryText, studentId, limit){
    limit = limit || 5;
    if(!_bank) return { ok:false, results:[], deflected:false, text:srT("smart_question_bank_not_loaded") };
    const student = (window.APP && APP.students || []).find(function(s){ return String(s.id)===String(studentId); }) || null;
    _lastMatchedStudent = student;
    const qTokens = tokenize(queryText);
    let candidates = flatQuestions().filter(q => q._categoryId==="per_student" && questionAiFeatureOk(q))
      .concat(availableQuestions());
    const scored = candidates.map(function(q){
      const labelTokens = tokenize(q.label);
      const catTokens = tokenize(q._categoryLabel);
      const keywordTokens = tokenize((q.keywords||[]).join(" "));
      let score = 0;
      ...
      if(score>0 && q._categoryId==="per_student") score += 3;
      return { question:q, score:score };
    }).filter(function(r){ return r.score>0; });
    scored.sort(function(a,b){ return b.score-a.score; });
    if(scored.length){
      const results = scored.slice(0,limit).map(function(r){
        return { id:r.question.id, label:r.question.label, category:r.question._categoryLabel, score:r.score };
      });
      return { ok:true, results:results, deflected:false, text:"" };
    }
    // No specific template scored (e.g. the original query was just the
    // student's name alone) — same overview-answer fallback match() uses.
    const overviewParts = ["rank_gap","trend","weakest_subject_student"]
      .map(function(id){ return answerQuestionImpl(id); })
      .filter(function(r){ return r.ok; })
      .map(function(r){ return r.text; });
    ...
    return { ok:true, results: candidates.slice(0,limit).map(function(q){ return { id:q.id, label:q.label, category:q._categoryLabel, score:0 }; }), deflected:false, text:"" };
  }
