import { esc, showScreen } from './app-utils-init.js';
import { computeCompareGroups, populateExportSectionPicker, selectCompareGroup, selectCompareSection } from './compute-compare.js';
...
import { renderExportPropertiesRail, renderShellLeftRail, renderShellRightRail, setRightRail, setShellRailOpen, setShellRailsOpen, smartQueryRailAnswer } from './vs-shell.js';
...
function buildDashboardControlsHtml(){
  ...
  const buckets=[
    ...
    {id:"compare",label:srT("bucket_compare_label"),desc:srT("bucket_compare_desc"),badge:null}
  ];
  ...
  if(APP.setup&&APP.setup.periodCount>1){
    buckets.push({id:"continuity",label:srT("bucket_continuity_label"),desc:srT("bucket_continuity_desc"),badge:null});
  }
...
function renderIndividualPlanAnswer(st){
  $("#bucket-answer-screen").html(`
    ...
    <div class="bucket-answer-body"><p>${esc(generateHomePlan(st,getStudentContinuityContext(st.id)))}</p></div>
  `).addClass("screen-fade-in").show();
}
