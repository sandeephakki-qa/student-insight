...
import { logCodeError } from './debug-log.js';
import { fitText, pdfRule, pdfT, PDF_THEME, sanitizePdfDoc, stampFooterAllPages } from './export-pdf.js';
...
import { buildScholarshipReportBytes } from './scholarship-export.js';
...
function criterionRows(result, schemeConfig) {
  ...
  rows.push({
    ...
    threshold: srT("scholarship_audit_max_absent_days", { days: schemeConfig.attendanceFloor }),
    ...
    ${
      !result.eligible && result.reasonCodes && result.reasonCodes.length
        ? `<div style="margin-top:12px;font-size:12.5px;color:var(--c-text2)"><b>${esc(
          ...
          )}:</b> ${esc(reasonText(result.reasonCodes))}</div>`
        : ""
    }
  </div>`;
}
...
function buildScholarshipCertificatePDF(doc, result, student, schemeConfig) {
  const s = APP.setup || {};
  const sch = (APP.setup && APP.setup.scholarship) || {};
  const W = 210, H = 297;
  const T = PDF_THEME;
  ...
  if (!result.eligible && result.reasonCodes && result.reasonCodes.length) {
    ...
    const reasonLine = pdfT("scholarship_audit_reason_label", "Reason") + ": " + reasonText(result.reasonCodes);
...
async function downloadScholarshipCertificates() {
  ...
  try {
    ...
    if (selected.length === 1 && !includeCommon) {
      ...
      const doc = sanitizePdfDoc(new jsPDF("p", "mm", "a4"));
      buildScholarshipCertificatePDF(doc, result, student, data.schemeConfig);
