...
| 2026-09-04 | new-feature/studinpro | core/render-i18n.js, ui/common/inline-actions.js, ui/smart-search/smart-query-v2-ui.js, core/app-utils-init.js, index.html, css/core.css, sw.js, core/studinpro-items.js (new), ui/common/studinpro-ticker.js (new), i18n/studinpro/{en,hi,kn,ta,te,mr,gu,bn,pa,ml,or,as,ur,de,fr,ru,zh}.json (new folder, 17 files) | Sandy-directed, built and iterated turn-by-turn over a long session (not an autonomous agent decision) — flagged explicitly against THE ONE RULE above because this is a visible Free-user UI change (new footer content replacing the old static "Student Insight — Open Source" line, a new clickable ticker, a new modal, a new About-page card); Sandy's own direct, repeated, explicit instruction across the whole build is the authorization this rule requires, equivalent to the "Sandy-authorized"/"Sandy-instructed" precedent elsewhere in this log. Feature: a footer ticker (`#studinpro-ticker`, full-width, JS-driven continuous right-to-left scroll via requestAnimationFrame — deliberately NOT CSS @keyframes/animationend, see file comment for why that approach was abandoned after repeated live-test failures) cycling through short StudInPro promo bullets (content lives entirely in core/studinpro-items.js — the one file to edit to add/remove a point — plus matching i18n/studinpro/en.json text; nothing else needs touching for future content changes), pausing fully on hover/touch with a "tap to learn more" hint, opening a shared-modal Google Form embed on click (also reachable from a new About-page "StudInPro" comparison card, same action/modal). Old static footer text and its `#footer-project-link` element removed entirely (not hidden) per Sandy's explicit instruction — `app-utils-init.js`'s `initEnvBadge()` selector updated to match. i18n wired the standard way (`studinpro` added to `I18N_FEATURES`, ticker re-renders on language switch via `reapplyI18nStrings()`) — English content is real/reviewed, all 16 other locale files (the existing 12 + prod's additional de/fr/ru/zh, generated fresh during this merge since prod supports more languages than the session this feature was built in) are English-content placeholders with correct `_meta.label`/`rtl` and `reviewed:false`, same convention as this repo's other unreviewed-locale entries — translation deliberately deferred to a later pass, not silently skipped (see i18n/studinpro/README.md). Two real bugs were found via actual live-Chromium testing (not static review) during the build and are already fixed in the delivered files: (1) `#footer{justify-content:center}` in css/vs-shell.css's existing desktop media query was centering the ticker at its ~140px natural content width instead of stretching it, silently collapsing the text-scroll viewport to 0px wide — fixed with an explicit `width:100%` on `#studinpro-ticker` (root-caused and confirmed via getBoundingClientRect() measurements before/after, not guessed); (2) `gsapModalEntrance` is an ES-module export from `render-core.js` never attached to `window` — code originally checked `typeof gsapModalEntrance === "function"` (always silently false), so the modal populated content but never got its visibility-triggering `.open` class; fixed with a real `import`. A third fix, also required and included: the CSP `<meta>` tag had no `frame-src` directive (falling back to `default-src 'self'`), which silently blocked framing the Google Form entirely — added `frame-src 'self' https://docs.google.com;`. Google Form submission "already submitted" memory uses one new localStorage key (`studinProFormSubmitted`, a boolean flag only — no student data, no PII beyond "this browser once submitted the interest form" — reviewed against this file's NO_PERSISTENCE hard constraint above and judged compliant since that constraint is specifically about student data, not app-level UI-state flags; flagging explicitly rather than silently assuming this is fine, since it's the first localStorage write this project has added). Verified end-to-end via a real headless-Chromium session in the actual delivered prod files (not just the build session's separate working copy) — jQuery/GSAP/xlsx/jsPDF/jsZip/Chart.js substituted with local npm-installed copies purely for the test harness (this sandbox's egress allowlist doesn't include the real CDNs), same class of workaround as the 2026-08-30 entry above, nothing touching the shipped repo: fresh page load → onboarding dismissed → ticker measured at full 1400px width (not the pre-fix ~140px) → clicked → modal opened with real English text (not raw i18n keys) → iframe present with Sandy's real Google Form URL → zero console errors. NOT verified: actual Google Form submission end-to-end (the "2nd iframe load = submitted" detection heuristic, chosen because Google Forms is cross-origin and exposes no real postMessage submit event) — this sandbox's network egress doesn't reach docs.google.com at all, confirmed via direct curl (403), so the real submit→confirmation-page→auto-close→"already submitted" flow needs Sandy's own manual test; also not verified, native-speaker fluency of anything (moot this pass, English-only content). This merge was done directly against Sandy's actual uploaded prod zip (not by copying the build session's own working folder over prod) — every file below was diffed against this repo's real current content first, matching the standing practice in every merge entry above. sw.js CACHE_VERSION bumped (v12.16→v12.17). |
